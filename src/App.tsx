...
export default function App() {
  ...
  const [allUsers, setAllUsers] = useState<UserProfile[]>([
    ...
    { uid: 'mock-uid-2', email: 'teki204666@gmail.com', displayName: 'Admin User 2', role: 'admin' },
    { uid: 'mock-staff-uid', email: 'staff@example.com', displayName: 'Staff User', role: 'staff' }
  ]);
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (firebaseUser) => {
      if (firebaseUser) {
        ...
        const registeredUser = allUsers.find(u => u.email.toLowerCase() === userEmail?.toLowerCase());
        if (registeredUser) {
          setUser({ uid: firebaseUser.uid, email: userEmail!, displayName: firebaseUser.displayName || 'User' });
  ...
  const handleLogin = async () => {
    try {
      ...
      if (registeredUser) {
        setUser({ uid: result.user.uid, email: userEmail!, displayName: result.user.displayName || 'User' });
        setProfile(registeredUser);
        setLoginError(null);
      } else {
        await signOut(auth);
        setLoginError(`Email (${userEmail}) 尚未註冊，請聯絡管理員。`);
      }
    } catch (error: any) {
      ...
      <div className="h-screen flex flex-col items-center justify-center bg-slate-50 p-6">
        ...
        >
          ...
          <Card className="p-8 space-y-6 shadow-lg shadow-slate-200/50 border-slate-200/60">
            ...
            <p className="text-sm text-slate-600">請使用已註冊的 Google 帳號登入系統以存取管理後台。</p>
            <div className="space-y-4">
              <Button onClick={handleLogin} className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white flex items-center justify-center gap-2">
              ...
              <div className="space-y-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <input 
                      ...
                      className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-sm transition-all" 
                    />
                    <p className="text-[10px] opacity-40 mt-1">當該使用者首次登入時，將自動套用此權限。</p>
                  </div>
                <div className="space-y-1">
